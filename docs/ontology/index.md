# Ontology

Ontology is the shared vocabulary layer for the ProtocolWarden ecosystem.

## Pages

- [Ontology vs Topology vs Topography](ontology-vs-topology-vs-topography.md)
- [Entities](entities.md)
- [Vocabulary](vocabulary.md)
- [Execution Objects](execution-objects.md)
- [Governance Objects](governance-objects.md)
- [Audit Objects](audit-objects.md)
- [Glossary Index](glossary-index.md)

## Purpose

The ontology section defines what the major concepts are, what they mean, and
which repos own them.

## Related

- [Architecture Charter](../architecture/platform-architecture-charter.md)
- [Topology](../topology/ecosystem-graph.md)
- [Glossary](../glossary/index.md)
