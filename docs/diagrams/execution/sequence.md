...
sequenceDiagram
    ...
    note over BE: TeamExecutor / DAGExecutor / CritiqueExecutor<br/>(multi-agent topologies)<br/>— or —<br/>DirectLocal / AiderLocal<br/>(single-agent via ExecutorRuntime subprocess layer)
...
- **DirectLocal / AiderLocal** are simpler single-agent adapters that use **ExecutorRuntime**
  as a subprocess mechanics library (process-group safety, timeout, stdout/stderr capture).
- ExecutorRuntime is a library dependency of the DirectLocal/AiderLocal adapters,
