...
| CxRP | contract_protocol | execution routing contract models | contract vocabulary for routing | runtime hosting, orchestration policy | OperationsCenter, SwitchBoard, TeamExecutor, DAGExecutor, CritiqueExecutor | repo license | active |
| RxP | contract_protocol | runtime invocation contract models | normalized runtime invocation vocabulary | runtime hosting, orchestration policy | OperationsCenter, ExecutorRuntime, TeamExecutor, DAGExecutor, CritiqueExecutor | repo license | active |
| OperationsCenter | control_plane | planning and orchestration consumer | control-plane coordinator | graph semantics, projection semantics | SwitchBoard, ExecutorRuntime (via direct_local/aider_local), TeamExecutor, DAGExecutor, CritiqueExecutor, Custodian | repo license | active |
...
| ExecutorRuntime | subprocess_mechanics | subprocess mechanics library for direct_local / aider_local OC adapters | process-group-safe execution, timeout, artifact capture | graph semantics, orchestration policy, AI topologies (TE/DE/CE do not use it) | OperationsCenter (direct_local, aider_local adapters only) | repo license | active |
